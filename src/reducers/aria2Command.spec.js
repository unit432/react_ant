...
describe("aria2Command reducer", () => {
  ...
  it("insert new command", () => {
    ...
    expect(aria2Command(currentState, addUri)).toEqual(newState);
  });
  ...
  it("replace existing command", () => {
    ...
    expect(aria2Command(currentState, updateTellStopped)).toEqual(newState);
  });
  ...
  it("handle clean aira2c command", () => {
    ...
    expect(aria2Command(currentState, cleanAira2Command)).toEqual(newState);
  });

  it("returns default state if action is unknow", () => {
    expect(aria2Command([], { type: "UNKNOW_ACTION" })).toEqual([]);
  });
});
